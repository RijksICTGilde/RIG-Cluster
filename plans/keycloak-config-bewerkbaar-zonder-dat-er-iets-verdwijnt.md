...
Status: uitgevoerd in RC-79 (PR #78), 12 augustus 2026. Plan geschreven op
...
## Hoe het is uitgevoerd (RC-79, PR #78)

- **Fase 0** — `additional_redirect_uris` kreeg een eigen fieldset;
  `tests/test_service_config_layout_coverage.py` is de poort, met
  `layout_field_names()` in `opi/forms/layout.py`. Verder geen enkele andere dienst met
  hetzelfde gat.
- **Fase 1** — elke gerenderde reeks post `_gerenderde-reeksen[]` met haar eigen
  formulierpad (`opi/templates_lotc/widgets/sequence.html.j2` +
  `opi/forms/editables/rendered_sequences.py`). Pad aanwezig = leeggemaakt, pad afwezig =
  ging er niet over, veld helemaal afwezig = geen uitspraak (oud gedrag, nodig voor de
  eindinzending van de wizard). Ook het snoeien van de originele rij slaat geneste
  reeksen over die niet getekend zijn.
- **Fase 2** — `TestDienstconfiguratieLijsten` in `test_modal_edit_nondestructive.py`.
- **Fase 3** — `restrict-access/role` en `account-link` zijn bewerkbaar; `variables`
  blijft er bewust buiten omdat `context.update(user_variables)` over de
  platformwaarden (`realm_name`, `platform_realm_name`, `platform_client_id`) heen gaat.
  Reden staat in `catalog/keycloak/help.md`, en is als toets vastgezet.
- **Fase 4** — `validation_alias=AliasChoices(...)` in het model én de tweede schrijfwijze
  in `KeycloakManager._get_keycloak_service_config`. Een alias die alleen valideert maar
  niet gelezen wordt is een stille no-op.
- **Bewust buiten scope** — de opslagmontages `components[*]/services{X}/config` hangen
  onder de dienstselectie van het component; wat daar kan spelen is de overlay van de
  componentenrij zelf, een eigen mechanisme.
