...
|---|---|---|---|
| A | Kritiek (opgelost, RC-98) | Image-push overschrijft de container-image van een ander project in de gedeelde registry, met code-executie in diens namespace als gevolg | `opi/connectors/skopeo.py:117`, `opi/api/image_router.py:30` |
...
**Uitkomst (RC-98, PR #97, 13 augustus 2026): opgelost door te pinnen.** De bestemming draagt nu het bezittende project voorop: `{REGISTRY_URL}/{REGISTRY_ORG}:{project}_{image_name}-{tag}`, opgebouwd in `build_registry_tag()` (`opi/utils/naming.py`) en gebruikt door `SkopeoConnector._build_destination`. De eigenaar komt uit de API-sleutel, niet uit het pad: `validate_api_token` overschrijft `project_name`. Het scheidingsteken is de underscore, die een projectnaam (`^[a-z][a-z0-9-]*$`) niet kan bevatten maar een image-naam wel, dus het deel vóór de eerste underscore is ondubbelzinnig de eigenaar. Een botsing tussen twee projecten is daarmee onmogelijk in plaats van verboden — optie 1 uit het plan, gekozen boven weigeren omdat weigeren een registry-lookup in het pushpad zou vragen en alsnog op naamvergelijking zou leunen.

De leeskant is meegenomen: `validate_platform_registry_image_ownership()` weigert een deployment-image dat naar de tag van een ander project in de platformregistry wijst. Die controle staat in `validate_project_structure`, het enige punt waar elke schrijfweg langskomt, en raakt uitsluitend verwijzingen naar `{REGISTRY_URL}/{REGISTRY_ORG}`; images van ghcr.io of een eigen registry blijven vrij, want een witte lijst van registries zou legitiem gebruik breken.

Migratie: bestaande tags worden niet hernoemd en niet verwijderd, dus draaiende deployments blijven bij een herstart dezelfde image trekken (van belang bij `imagePullPolicy: Always`). Tags zonder eigenaar-prefix blijven leesbaar; ze zijn alleen niet meer beschrijfbaar, want elke push is voortaan geprefixt. Aangetoond met twee projecten en twee echte sleutels in `tests/test_tenant_isolation_registry.py`. Zie `features/registry-image-ownership.md`, inclusief het restrisico op een oude platte tag waarvan de image-naam toevallig met `{projectnaam}_` begint.
