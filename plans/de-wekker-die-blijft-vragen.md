...
Gemeten op 17 augustus 2026, naar aanleiding van een waarneming in productie: er komt een gestage stroom `GET /api/sleep-mode/{p}/{d}/status` binnen terwijl er niemand een wekkerpagina open heeft.
...
## Twee richtingen, en ze zijn geen alternatieven

**A. De pod pollt alleen als er iemand wacht.** Dit is de echte reparatie en het scheelt vrijwel alles. De wekkerpod weet wanneer er een browsertab hangt (die pollt hém) en wanneer hij zelf een wek-verzoek heeft gestuurd. Buiten die twee gevallen is er niemand voor wie het antwoord uitmaakt.

**B. OPI houdt het antwoord even vast.** Een korte cache op het statusendpoint, zodat tien wekkers die dezelfde deployment volgen samen één kubectl kosten.

B is geen tweede keus maar een andere bescherming, en dat verschil moet in het ontwerp staan: **het waker-image wordt los uit de registry gepulld.** Er draaien dus pods met een oudere versie die na deze wijziging gewoon op 3 seconden blijven pollen, en die bereik je alleen met B. A beschermt tegen het gedrag, B beschermt tegen de images die je niet in de hand hebt. Doe allebei, en zeg per stuk wat het afvangt.
...
Dat is de enige manier waarop deze wijziging iets kan breken, en het is een ernstige. Elk voorstel voor richting A moet expliciet beantwoorden hoe de pod een wekactie van buitenaf opmerkt. Denkbare wegen, in oplopende omvang:
...
**Het contract blijft staan.** `state` op `/status` levert `starting | ready` en dat is byte-identiek wat een oud waker-image leest (`main.go:162`). Niet aanraken. RC-119 heeft er een tweede veld naast gezet met de echte slaaptoestand; dat is de plek voor iets nieuws.

**Een eigen uitrol.** Een wijziging in `images/zad-waker/` is een nieuw image in de registry en dus een aparte uitrolstap, los van OPI. Beschrijf die stap, en wat er gebeurt in het venster waarin oude en nieuwe wekkers naast elkaar draaien.
...
- Een test op de cache in OPI: twee opeenvolgende pollverzoeken kosten één kubectl-aanroep.
