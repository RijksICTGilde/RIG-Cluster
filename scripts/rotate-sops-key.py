...
def show_plan(plan: RotationPlan) -> None:
    ...
    for path in plan.sops:
        print(f"  sops rotate  {path.relative_to(REPO)}")
    print(f"\n{len(plan.env)} loose base64+age: values on the old key:")
    for field_ in plan.env:
        print(f"  recrypt      {field_.path.relative_to(REPO)}:{field_.line_number}  {field_.name}")
...
async def run_rotation(plan: RotationPlan, old_private: str, old_public: str, new_public: str) -> None:
    for path in plan.sops:
        ...
        print(f"  converted  {path.relative_to(REPO)}")
    for field_ in plan.env:
        ...
        print(f"  converted  {field_.path.relative_to(REPO)}:{field_.line_number} {field_.name}")
