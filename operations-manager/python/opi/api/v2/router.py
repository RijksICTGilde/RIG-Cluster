...
class ServiceValuesRead(BaseModel):
    ...
    values: dict[str, str] = Field(
        ...,
        description=(
            "Name -> value, in the same shape a POST to this path writes. **The values of "
            "`user-env-vars` are never returned, because they can hold secrets; every one of "
            "them comes back as '***'. You can set and change them, you cannot read them back, "
            "and there is no flag or option that lifts that.** The names are what makes the map "
            "readable, and they are what this endpoint is for. `aliases` is the exception and "
            "the reason is in the values: an alias is a reference to a platform variable "
            "(`$DATABASE_SERVER_HOST`), so the value IS the coupling and is no secret at all -- "
            "masking it would hide exactly what the reader asked about."
        ),
...
def _make_values_read_endpoint(service_name: str, service, layer: ConfigLayer):
    ...
    **The request never gets a say in that, and that is the point.** An env-var value can
    hold a secret, so it is masked, full stop -- no query parameter, no body flag, no
    per-value marking. A read path is exactly as easy to reach for an automated client
    that has been talked into asking for one as it is for the person who owns the
    project, and being unable to check a value you just wrote is the price of that. If
    you are here to add an exception, read ``Service.owned_value_is_secret`` first: the
    trade was made there, deliberately, and this endpoint is only the place it shows.
    """
...
def _values_read_description(service_name: str, layer: ConfigLayer) -> str:
    ...
    secrets = (
        "**The values of `user-env-vars` are never returned, because they can hold secrets. You "
        "can set and change them through this path; you cannot read them back.** Every one of "
        "them comes back as `***`, there is no flag, option or per-value exception that lifts "
        "that, and asking again does not change the answer. The names always come back, and "
        "they are what makes this endpoint useful: which variables a component has, and whether "
        "the one you just wrote is there.\n\n"
        "`aliases` does return its values, and the difference is in what those values are: an "
        "alias is a reference to a platform variable (`$DATABASE_SERVER_HOST`), so the value IS "
        "the coupling and holds no secret -- masking it would hide the one thing a reader is "
        "checking. An alias value that is not a reference is masked all the same. The owning "
        "service decides, per value; the answer is never negotiable from the request."
    )
    return "\n".join(
        [
