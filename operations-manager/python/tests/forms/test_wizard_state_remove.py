...
def test_step_data_shrinks_when_not_also_in_template() -> None:
    """Without the shadow copy in base_data, get_merged_data naturally
    ...
    state = WizardState(
        ...
        project_name="pm-5sj",
        # The fix: base_data does NOT carry 'users' — it's fully owned by the
        # team section's editable, so step_data is the single source of truth.
        base_data={"name": "pm-5sj", "deployments": [{"name": "prod"}]},
    )
...
def test_merge_by_index_still_works_for_shared_non_owned_lists() -> None:
    ...
    state = WizardState(
        ...
        # in base_data and get_merged_data combines fields.
        base_data={
...
def test_fully_owned_list_keys_ignores_indexed_paths() -> None:
    ...
    and their lists must stay in base_data so merge-by-index preserves
