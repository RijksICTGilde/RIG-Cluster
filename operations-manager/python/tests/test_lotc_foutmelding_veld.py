...
- dat onze kopie van ``components/_forms.j2`` op precies de bedoelde punten van de
  geinstalleerde afwijkt, zodat een nieuwe versie van lotc-forms hier opvalt en niet
  stilletjes langs ons heen gaat.
...
def test_onze_kopie_wijkt_op_precies_die_punten_af() -> None:
...
def test_de_kopie_wint_van_de_geinstalleerde() -> None:
    """Zonder deze volgorde doet onze kopie niets en valt de reparatie stil weg."""
    geladen = templates_lotc.env.get_template("components/_forms.j2").filename
    assert geladen == str(ONZE_KOPIE), f"components/_forms.j2 komt van {geladen}"
