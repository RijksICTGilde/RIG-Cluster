...
class TestTheNonSecretMarking:
    """punt 5: what the marking does to the project file, and what it deliberately does not.

    The one thing worth stating twice: it does not move a value out of the encrypted
    block. A marked value is encrypted in ``zad-projects`` exactly like an unmarked one,
    and the only plaintext the marking adds is the NAME -- which a read has always
    returned anyway.
    """

    @pytest.mark.asyncio
    async def test_the_marked_names_land_next_to_the_block(self, manager) -> None:
        instance, commit = manager
        ...
        await instance.set_component_values(
            ...
            "add",
            component_name="backend",
            values={"APP_MODE": "productie", "TOKEN": "geheim"},
            public=["APP_MODE"],
        )

        assert _component(commit.data)["user-env-vars-public"] == ["APP_MODE"]

    @pytest.mark.asyncio
    async def test_a_marked_value_is_still_encrypted(self, manager) -> None:
        instance, commit = manager
        ...
        await instance.set_component_values(
            ...
            "add",
            component_name="backend",
            values={"APP_MODE": "productie"},
            public=["APP_MODE"],
        )

        component = _component(commit.data)
        assert "productie" not in str(component["user-env-vars"]), "the marking put a value in the clear"
        assert decode(component["user-env-vars"], commit.data) == {"APP_MODE": "productie"}

    @pytest.mark.asyncio
    async def test_writing_without_the_marking_stores_no_list(self, manager) -> None:
        instance, commit = manager

        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"TOKEN": "geheim"}
        )

        assert "user-env-vars-public" not in _component(commit.data)

    @pytest.mark.asyncio
    async def test_rewriting_a_value_without_the_marking_makes_it_a_secret_again(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"A": "1"}, public=["A"]
        )

        await instance.set_component_values(
            "user-env-vars", "component", "patch", component_name="backend", values={"A": "2"}
        )

        assert "user-env-vars-public" not in _component(commit.data), "an omitted flag may only ever hide more"

    @pytest.mark.asyncio
    async def test_marking_another_value_leaves_the_first_marking_alone(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"A": "1"}, public=["A"]
        )
        ...
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"B": "2"}, public=["B"]
        )

        assert _component(commit.data)["user-env-vars-public"] == ["A", "B"]

    @pytest.mark.asyncio
    async def test_deleting_a_value_takes_its_marking_with_it(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars",
            "component",
        ...
        await instance.set_component_values(
            "user-env-vars", "component", "delete", component_name="backend", keys=["A"]
        )

        # Otherwise a later value that happens to get the name A would be shown because
        # of a decision somebody made about a different value.
        assert _component(commit.data)["user-env-vars-public"] == ["B"]

    @pytest.mark.asyncio
    async def test_clearing_removes_the_marking_too(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"A": "1"}, public=["A"]
        )
    ...
    @pytest.mark.asyncio
    async def test_marking_an_existing_value_is_a_change_worth_committing(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"A": "1"}
        )
        ...
        result = await instance.set_component_values(
            "user-env-vars", "component", "patch", component_name="backend", values={"A": "1"}, public=["A"]
        )

        assert result["changed"] is True
        assert len(commit.commits) == 2
        assert _component(commit.data)["user-env-vars-public"] == ["A"]
        # The values did not move, so their ciphertext must not be rewritten either.
        assert _component(commit.data)["user-env-vars"] is before

    @pytest.mark.asyncio
    async def test_re_marking_what_is_already_marked_commits_nothing(self, manager) -> None:
        instance, commit = manager
        await instance.set_component_values(
            "user-env-vars", "component", "add", component_name="backend", values={"A": "1"}, public=["A"]
        )
    ...
    @pytest.mark.asyncio
    async def test_the_deployment_layer_keeps_its_own_marking(self, manager) -> None:
        instance, commit = manager
        ...
        assert _component(commit.data)["user-env-vars-public"] == ["A"]
        assert "user-env-vars-public" not in _deployment_component(commit.data)
