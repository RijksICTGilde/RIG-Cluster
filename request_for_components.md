...
## 7. De foutmelding bij een formulierveld is onzichtbaar (lotc-forms)
...
## 8. `nldd-list dividers="never"` tekent de lijnen toch
...
## 9. "Optioneel" staat op elk veld dat niet `required` is
...
- betekent "Optioneel" niets, en bij een herhaalbaar item ("URI Optioneel") leest het als
ruis.

**Wat wij intussen doen.** Die velden `required` noemen. Dat is een omweg: het label
verdwijnt, maar de HTML zegt nu dat er iets ingevuld MOET worden, en dat is bij een
kiezer met een vaste selectie een andere leugen dan de vorige.
