...
- **`task fundament:install-platform` bestaat niet meer.** Die installeerde ingress-nginx, cert-manager, local-path-provisioner en CloudNativePG imperatief, en sinds de app-of-apps-opzet doen de vier Applications op wave -1 exact hetzelfde, met dezelfde gepinde versies en dezelfde upstream-URL's. Twee installatiepaden voor dezelfde CRD's leverden een SharedResourceWarning op en twee eigenaren voor één resource. Nu is `task cluster:bootstrap` het enige installatiepad: die zet ArgoCD neer en de rest komt uit git.
